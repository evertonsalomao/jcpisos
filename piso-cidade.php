<?php
// Carrega dinamicamente a página de piso intertravado baseada no slug da URL
require_once 'qube-manager/config/database.php';

// Pega o slug da URL (remove "piso-intertravado-em-" do início)
$slug = $_GET['slug'] ?? '';
...
ob_start();
include('piso-intertravado-seo.php');
...
$content = preg_replace(
    '/<title>.*?<\/title>/',
    '<title>Piso Intertravado em ' . $cityName . ' - SP - JC Pisos Intertravados de Concreto</title>',
    $content
);
...
$content = preg_replace(
    ...
    '<meta name="description" content="Piso intertravado de concreto em ' . $cityName . '. Soluções completas para obras residenciais, comerciais e industriais. Orçamento grátis!">',
    $content
);
